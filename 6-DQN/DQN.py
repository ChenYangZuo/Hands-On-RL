...
# @Time : 2023/2/24 下午2:09:05
# @Author : ZZZCY
# @File : [Hands-On-RL] --> DQN.py
...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):  # 将(SARSA)数据加入buffer
...
class DQN:
    ...
    def load(self):
        self.q_net.load_state_dict(torch.load("./save_mode/model.pth"))
    
    def save(self):
        folder = "./save_mode"
        if not os.path.exists(folder):
            os.mkdir(folder)
        torch.save(self.q_net.state_dict(), folder + "/model.pth")
